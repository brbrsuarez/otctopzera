...
class LauncherCore:
	def __init__(self, tibia_dir: str | None = None):
		...
		self.target_folder_name = 'OTCTOPZERA'
	...
	def get_default_tibia_directory(self):
		...
		if appdata and os.path.isdir(appdata):
			preferred = os.path.join(appdata, "OTCTOPZERA")
		...
		legacy_paths = [
			os.path.expandvars(r"%USERPROFILE%\Documents\OTCTOPZERA"),
			os.path.expandvars(r"%PROGRAMFILES%\OTCTOPZERA"),
			os.path.expandvars(r"%PROGRAMFILES(X86)%\OTCTOPZERA"),
			os.path.join(os.getcwd(), "OTCTOPZERA")
		]
		for path in legacy_paths:
			if os.path.exists(path):
				return path
		return os.path.join(os.getcwd(), "OTCTOPZERA")
	...
	def launch_tibia(self):
		...
		exe_names = ['otclient.exe', 'client.exe', 'Tibia.exe', 'tibia.exe', 'TibiaClient.exe']
		...
		raise Exception("Could not find client executable. Expected: OTCTOPZERA/bin/otclient.exe or OTCTOPZERA/otclient.exe")
